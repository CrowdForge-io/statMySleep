...
def choose_option():
    ...
    if choose == '1':
        ...
        plotsee = input("continue to your plot and the stats? (y/n)")
        if plotsee == 'y':
            sleep_plot()
            average_week()
            print("overview of your stats: \n")
            see_week()
            create_write = input("wanna write to your file? (y/n):  ")
            if create_write == 'y':
                try:
                    write_csv()
                except:
...
def main():
    ...
    start_p = input("stay or leave the program (s/l):  ")
    while start_p == 's':
        choose_option()
    else:
        sys.exit()
